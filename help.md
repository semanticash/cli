...
| `intent-gap` | `enabled\|disabled\|on\|off\|true\|false` | Allow or disallow manual intent-gap analysis in this repository |
...
current branch. It runs in the foreground so analysis or upload failures are
visible immediately.
...
| `intent_gap_enabled` | Allows manual local intent-gap analysis for open pull requests. Defaults to `false`. |
