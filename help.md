...
Some global state also lives under Semantica's global home directory
(`$SEMANTICA_HOME`), including the broker registry. When the launcher is
enabled, `$SEMANTICA_HOME/worker-launcher.log` records launcher-level events
while per-repo job output continues to go to `.semantica/worker.log`.
