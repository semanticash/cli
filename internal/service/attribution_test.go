...
func TestBuildNormalizedSet(t *testing.T) {
	...
	// "func foo() {" and "func foo(){" should both normalize to "funcfoo(){"
	if _, ok := norm["main.go"]["funcfoo(){"]; !ok {
		t.Error("expected normalized set to contain 'funcfoo(){'")
	}
	if _, ok := norm["main.go"]["returnx+y"]; !ok {
		t.Error("expected normalized set to contain 'returnx+y'")
	}
}

func TestMatchTier2_FormatterChanges(t *testing.T) {
	// AI wrote "func foo(){" but formatter changed to "func foo() {"
	aiLines := map[string]map[string]struct{}{
		"main.go": {
			"func foo(){": {},
		},
	...
	// Exact match fails
	if _, ok := aiLines["main.go"]["func foo() {"]; ok {
		t.Fatal("exact match should NOT work here")
	}

	// Normalized match succeeds
	norm := normalizeWhitespace("func foo() {")
	if _, ok := aiLinesNorm["main.go"][norm]; !ok {
		t.Error("normalized match should work for formatter-changed line")
	}
}
...
func TestParseDiff_GroupsByContiguousAddedLines(t *testing.T) {
	...
	if len(dr.files) != 1 {
		t.Fatalf("expected 1 file, got %d", len(dr.files))
	}
	fd := dr.files[0]
	if fd.path != "main.go" {
		t.Fatalf("expected path main.go, got %s", fd.path)
	}
	if len(fd.groups) != 2 {
		t.Fatalf("expected 2 groups, got %d", len(fd.groups))
	}
	// Group 1: import lines
	if len(fd.groups[0].lines) != 2 {
		t.Errorf("group 0: expected 2 lines, got %d", len(fd.groups[0].lines))
	}
	// Group 2: function body lines
	if len(fd.groups[1].lines) != 2 {
		t.Errorf("group 1: expected 2 lines, got %d", len(fd.groups[1].lines))
	}
}

func TestHunkOverlap_AIModifiedInOverlappingGroup(t *testing.T) {
	// A contiguous group of 4 lines where 3 match AI output and 1 doesn't.
	// The non-matching line should be AI-Modified, not Human.
	aiLines := map[string]map[string]struct{}{
		"main.go": {
			"func handler() {":       {},
			"fmt.Println(\"hello\")": {},
	...
	diff := strings.Join([]string{
		...
		"+func handler() {",
		"+\tfmt.Println(\"hello\")",
		"+\tlog.Debug(\"user added this\")",
		"+\treturn nil",
		" }",
	}, "\n")

	dr := parseDiff([]byte(diff))
	if len(dr.files) != 1 || len(dr.files[0].groups) != 1 {
		t.Fatalf("expected 1 file with 1 group, got %d files", len(dr.files))
	}

	group := dr.files[0].groups[0]
	fp := dr.files[0].path

	// Two-pass classification (mirrors the real algorithm).
	type lineClass struct{ tier int }
	...
	for _, line := range group.lines {
		...
		if lc.tier == 0 {
			...
			if normSet, ok := aiLinesNorm[fp]; ok {
	...
	if !hasOverlap {
		t.Fatal("expected group to have overlap")
	}

	var exact, formatted, modified, human int
	...
	if modified != 1 {
		t.Errorf("expected 1 modified (the user-added line), got %d", modified)
	}
	if human != 0 {
		t.Errorf("expected 0 human in overlapping group, got %d", human)
	}
}

func TestHunkOverlap_HumanGroupNoOverlap(t *testing.T) {
	// All lines in the group are human - no AI matches at all.
	// Every line should be classified as Human.
	aiLines := map[string]map[string]struct{}{
		"main.go": {
	...
	if hasOverlap {
		t.Fatal("expected no overlap in human-only group")
	}

	for i, lc := range classes {
		if lc.tier != 0 {
			t.Errorf("line %d: expected tier 0 (human), got tier %d", i, lc.tier)
		}
	}
}

func TestHunkOverlap_MixedGroupsInSameFile(t *testing.T) {
	// Two groups in one file: first group overlaps AI, second is pure human.
	// They should be classified independently.
	aiLines := map[string]map[string]struct{}{
		"main.go": {
			"fmt.Println(\"ai line\")": {},
		},
	...
	// Process each group independently (mirrors real algorithm).
	var totalExact, totalModified, totalHuman int
	...
	// Group 1: "ai line" (exact) + "userEditedLine()" (modified) = 1 exact, 1 modified
	// Group 2: "pureHumanCode()" + "morePureHuman()" = 2 human
	if totalExact != 1 {
		t.Errorf("expected 1 exact, got %d", totalExact)
	}
	if totalModified != 1 {
		t.Errorf("expected 1 modified, got %d", totalModified)
	}
	if totalHuman != 2 {
		t.Errorf("expected 2 human, got %d", totalHuman)
	}
}

func TestHunkOverlap_FormattedLineEstablishesOverlap(t *testing.T) {
	// AI wrote "func foo(){" - formatter changed to "func foo() {".
	// The normalized match establishes overlap, so the unmatched line
	// in the same group becomes AI-Modified.
	aiLines := map[string]map[string]struct{}{
		"main.go": {
			"func foo(){": {},
		},
	...
	if !hasOverlap {
		t.Fatal("expected overlap via normalized match")
	}

	var formatted, modified int
	for _, lc := range classes {
		switch {
		case lc.tier == 2:
			formatted++
		case hasOverlap && lc.tier == 0:
			modified++
		}
	}

	// "func foo() {" -> formatted (normalized match)
	// "userAddedLine := true" -> modified (in overlapping group)
	// "}" -> blank after trim? No, "}" is not blank. -> modified
	if formatted != 1 {
		t.Errorf("expected 1 formatted, got %d", formatted)
	}
	if modified != 2 {
...
func TestAttribution_CursorFileLevelAttribution(t *testing.T) {
	...
	paths := extractProviderFileTouches(ev.ToolUses)
...
func TestHasProviderFileEdit_KiroFileEdit(t *testing.T) {
	// Kiro CLI emits this exact JSON shape from BuildToolUsesJSON.
	kiroPayload := sql.NullString{
		String: `{"tools":[{"name":"kiro_file_edit","file_path":"alpha.txt","file_op":"create"}]}`,
		Valid:  true,
	}
	if !hasProviderFileEdit(kiroPayload) {
		t.Error("hasProviderFileEdit should recognize kiro_file_edit")
	}

	paths := extractProviderFileTouches(kiroPayload)
	if len(paths) != 1 || paths[0] != "alpha.txt" {
		t.Errorf("extractProviderFileTouches = %v, want [alpha.txt]", paths)
	}
}

func TestHasProviderFileEdit_UnknownTool(t *testing.T) {
	payload := sql.NullString{
		String: `{"tools":[{"name":"unknown_tool","file_path":"file.txt"}]}`,
		Valid:  true,
	}
	if hasProviderFileEdit(payload) {
		t.Error("hasProviderFileEdit should not recognize unknown_tool")
	}
}

func TestHasProviderFileEdit_Empty(t *testing.T) {
	if hasProviderFileEdit(sql.NullString{}) {
		t.Error("hasProviderFileEdit should return false for empty NullString")
	}
}
