...
// IsFullCommitHash reports whether s is a full lowercase SHA-1 or SHA-256
// object ID.
...
// nulSeparatedPaths splits NUL-separated git path output into
// OS-normalized relative paths.
func nulSeparatedPaths(out []byte) []string {
	parts := bytes.Split(out, []byte{0})
	...
	for _, p := range parts {
		...
		paths = append(paths, filepath.FromSlash(string(p)))
	}
	return paths
}

// ChangedPathsBetween returns paths that differ between two commit trees.
...
func (r *Repo) ChangedPathsBetween(ctx context.Context, fromCommit, toCommit string) ([]string, error) {
	if !IsFullCommitHash(fromCommit) || !IsFullCommitHash(toCommit) {
		return nil, fmt.Errorf("changed paths: want full commit hashes, got %q..%q", fromCommit, toCommit)
	}
	cmd := r.gitCmd(ctx, "diff", "--name-only", "--no-renames", "-z", fromCommit, toCommit)
	out, err := cmd.Output()
	if err != nil {
		if ee, ok := errors.AsType[*exec.ExitError](err); ok {
			return nil, fmt.Errorf("git diff %s..%s failed: %w: %s", fromCommit, toCommit, err, string(ee.Stderr))
		}
		return nil, fmt.Errorf("git diff %s..%s failed: %w", fromCommit, toCommit, err)
	}
	return nulSeparatedPaths(out), nil
}

// TrackedFilesAtCommit returns the paths tracked in the given commit's
// tree. The id must be a full commit hash.
func (r *Repo) TrackedFilesAtCommit(ctx context.Context, commit string) ([]string, error) {
	if !IsFullCommitHash(commit) {
		return nil, fmt.Errorf("tracked files: want a full commit hash, got %q", commit)
	}
	cmd := r.gitCmd(ctx, "ls-tree", "-r", "--name-only", "-z", commit)
	out, err := cmd.Output()
	if err != nil {
		if ee, ok := errors.AsType[*exec.ExitError](err); ok {
...
// ChangedPathsFromCommitToWorktree returns staged, unstaged, and deleted
// paths relative to commit. It overrides Git settings that weaken stat
// validation. On Unix, ctime and inode changes expose same-size rewrites
// with restored mtimes. Windows has no equivalent signal, so callers that
// require this guarantee must disable reuse there. Git still skips
// assume-unchanged and skip-worktree entries; use OrdinaryTrackedFiles to
// exclude them.
func (r *Repo) ChangedPathsFromCommitToWorktree(ctx context.Context, commit string) ([]string, error) {
	if !IsFullCommitHash(commit) {
		return nil, fmt.Errorf("worktree diff: want a full commit hash, got %q", commit)
	}
	cmd := r.gitCmd(ctx,
	...
	if err != nil {
		if ee, ok := errors.AsType[*exec.ExitError](err); ok {
			return nil, fmt.Errorf("git diff %s (worktree) failed: %w: %s", commit, err, string(ee.Stderr))
		}
		return nil, fmt.Errorf("git diff %s (worktree) failed: %w", commit, err)
	}
	return nulSeparatedPaths(out), nil
}

// OrdinaryTrackedFiles returns index paths tagged "H" by git ls-files -v.
// It excludes entries whose working-tree content Git does not verify,
// including assume-unchanged, skip-worktree, unmerged, and removed entries.
func (r *Repo) OrdinaryTrackedFiles(ctx context.Context) ([]string, error) {
	cmd := r.gitCmd(ctx, "ls-files", "--cached", "-v", "-z")
	out, err := cmd.Output()
	if err != nil {
		if ee, ok := errors.AsType[*exec.ExitError](err); ok {
			return nil, fmt.Errorf("git ls-files --cached -v failed: %w: %s", err, string(ee.Stderr))
		}
		return nil, fmt.Errorf("git ls-files --cached -v failed: %w", err)
	}
	// Each NUL-separated record is "<tag> <path>".
	parts := bytes.Split(out, []byte{0})
	paths := make([]string, 0, len(parts))
	for _, p := range parts {
		if len(p) < 3 || p[1] != ' ' {
		...
		paths = append(paths, filepath.FromSlash(string(p[2:])))
	}
	return paths, nil
}
