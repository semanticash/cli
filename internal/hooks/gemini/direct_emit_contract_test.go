...
func TestDirectEmit_Contract(t *testing.T) {
	...
	cases := []testutil.Case{
		...
		},
		{
			...
			Description: "Gemini 0.40 invoke_agent BeforeTool: prompt + agent_name",
			Event: &hooks.Event{
		...
		{
			Name:        "subagent_completed_success",
			Description: "Gemini 0.40 invoke_agent AfterTool: returnDisplay.state=success",
			Event: &hooks.Event{
				...
				ToolName:      "Agent",
				ToolInput:     json.RawMessage(`{"prompt":"Review the auth package","agent_name":"codebase_investigator"}`),
				ToolResponse:  json.RawMessage(`{"llmContent":"Reviewed: found two issues in auth/session.go","returnDisplay":{"isSubagentProgress":true,"agentName":"codebase_investigator","state":"success"}}`),
				Timestamp:     1714000050000,
			},
		},
		{
			Name:        "subagent_completed_error",
			Description: "Gemini 0.40 invoke_agent AfterTool: returnDisplay.state=error (quota / runtime failure)",
			Event: &hooks.Event{
				Type:          hooks.SubagentCompleted,
				SessionID:     "sess-gemini-1",
				TranscriptRef: transcriptRef,
				...
				ToolResponse:  json.RawMessage(`{"llmContent":"Subagent 'codebase_investigator' failed. Error: ...","returnDisplay":{"isSubagentProgress":true,"agentName":"codebase_investigator","state":"error"}}`),
