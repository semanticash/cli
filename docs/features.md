...
- Historical carry-forward applies to created files present in the previous commit-linked manifest and to modified files whose committed content matches an earlier workspace observation, such as a manual `semantica checkpoint`. Missing or mismatched observations fail closed.
