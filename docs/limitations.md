...
- Historical carry-forward applies to created files present in the previous commit-linked manifest and to modified files whose committed content matches an earlier unlinked workspace observation, such as a manual `semantica checkpoint`. Modified-file evidence is limited to that observation's event window and does not replace current-window line attribution. Missing or mismatched observations fail closed.
