...
- Arbitrary shell mutation destinations remain unresolved without trustworthy path evidence. Retrying later cannot reconstruct a missing before-state. Existing single-repository snapshots continue; no additional repositories are scanned.
- Unresolved records remain local under `$SEMANTICA_HOME/unresolved-mutations/`. They are not automatically routed, uploaded, or deleted. Read-only Bash commands are included because they also lack mutation paths. `semantica doctor` reports the retained count as informational/OK; an unreadable archive warns. The count is not a misattribution rate.
- Retention has no size cap or automatic cleanup. Each record embeds its referenced objects, even when shared with other events, so storage grows with usage. This initial implementation favors preserving evidence over discarding it; indefinite unattended use needs a retention policy. A local delta does not prove there were no effects elsewhere, so it does not remove a retained record. Existing historical records and completed attribution results are unchanged.
