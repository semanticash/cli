...
These are potential mutation events, not confirmed file changes. Read-only Bash commands also lack mutation paths. Known delegation boundaries (`Agent` and supported provider aliases) remain session context, including completions marked `exec`. Unknown tools are treated conservatively. `semantica doctor` reports the global retained count as informational/OK, not a misattribution rate. An unreadable archive still produces a warning.
...
- This initial implementation accepts unbounded local storage growth to avoid silently discarding unresolved evidence. Each record embeds its referenced objects, including objects shared by other events. There is no size cap or automatic cleanup; long-running installations can accumulate substantial data. A retention policy remains necessary before recommending indefinite unattended use.
