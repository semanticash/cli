...
- Frozen upload fixtures retain their exact bytes on Windows checkouts, preserving content hashes.
- Attribution now credits matching lines from complete turn snapshots for Codex, Claude Code, Gemini CLI, Copilot CLI, Cursor IDE/interactive CLI, and Kiro CLI v2, including cross-repository shell changes. Direct edit and tool-delta matches take precedence; snapshot-based credit is marked as inferred.
...
- Packaged turn provenance now preserves available provider-reported token
  usage for hosted PR Journey and turn-detail views, including Cursor turns
  whose transcript contains no new assistant event.
...
- Events with unresolved mutation destinations and their referenced evidence
  ...
  reports the global retained count as informational, including shell commands
  without mutation paths. Records are not automatically routed or deleted;
  local storage can grow without a bound.
...
- `semantica doctor`, `semantica status`, and `semantica tidy` now detect stale broker registry entries whose local `.semantica` state was deleted or reset. Confirmed stale entries can be pruned with `semantica tidy --apply`, and user-global hooks skip them without recording hook-error warnings.
