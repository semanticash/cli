...
- Added a bounded workspace snapshot and delta engine with a crash-safe registry and bounded store maintenance for concurrent agent-run shell tools. It stores deterministic text hunks and file-level evidence in an isolated Git store without contacting repository remotes. Shell-tool deltas are not yet included in attribution results.
